...
def build_split_name(
    ...
    """
    Build a readable split name.

    ``__AND__`` means that all named feature conditions are used
    to define train. Multiple values from the same feature are OR
    conditions, while different features are AND conditions.

    For example:

        Mandrel retraction timing in [5, 10]

    means timing is either 5 or 10.
    """
    ...
    for (
        feature,
        values,
    ) in values_by_feature.items():
        clean_values = [
            clean_scalar_for_name(value)
            for value in values
        ]

        if len(clean_values) == 1:
            condition_names.append(
                f"{feature}={clean_values[0]}"
            )
            continue
...
def collect_train_group_ids_from_conditions(
    bending_setups_df: pd.DataFrame,
    selected_conditions: tuple[dict, ...],
) -> list[int]:
    """
    Return groups matching the selected split conditions.

    Values from the same feature are OR conditions. Different features
    are AND conditions.
    """
    values_by_feature: dict[
        str,
...
def generate_split_metadata(
    ...
    *,
    ...
    """
    Generate one metadata row per unique train/test split.

    Candidate definitions include:

      - all single atomic conditions;
      - all two-condition combinations;
      - all three-condition combinations.

    Conditions from the same feature are allowed and are treated as
    alternatives. For example:

        Mandrel retraction timing=5
        __AND__
        Mandrel retraction timing=10

    means timing is either 5 or 10. Conditions across different
    features must all match. The matching groups are assigned to train.
    Every remaining group is assigned to test.
    """
    ...
    if minimum_combination_size < 1:
        raise ValueError(
            "minimum_combination_size must be at least 1."
        )
    ...
    if (
        ...
        raise ValueError(
            "maximum_combination_size must be greater than "
            "or equal to minimum_combination_size."
        )

    bending_setups_df = (
        bending_setups_df.copy()
    )
    ...
    # A single train-group set can be produced by multiple textual
    # condition combinations. Keep only the first deterministic one.
    ...
    for condition_count in range(
        ...
        for selected_conditions in combinations(
            atomic_conditions,
            condition_count,
        ):
            train_group_ids = (
                collect_train_group_ids_from_conditions(
                    bending_setups_df=(
                        bending_setups_df
                    ),
                    selected_conditions=(
            ...
            test_group_ids = sorted(
                set(all_group_ids).difference(
                    train_group_ids
                )
            )

            # Full-train or full-test candidates are invalid.
            ...
            if experiment_overlap:
                raise ValueError(
                    "Experiment leakage detected between train "
                    "and test groups. Overlapping experiments: "
            ...
            observed_split_keys.add(
                split_key
            )

            split_name = build_split_name(
                selected_conditions
            )
    ...
    if not candidate_rows:
        raise ValueError(
            "No valid train/test splits were generated."
        )
    ...
    # Deterministic order:
    # single -> pair -> triple, then alphabetical split name.
    split_metadata_df = (
        split_metadata_df
        .sort_values(
            [
    ...
    split_metadata_df.insert(
        ...
        range(
            1,
            len(split_metadata_df) + 1,
        ),
    )

    # Maintained for compatibility with the existing pipeline.
    split_metadata_df.insert(
        1,
        "summary_df_index",
        range(
            len(split_metadata_df)
        ),
    )
...
def run_qrf_split_creating(
    *,
    ...
    print(
        "Combination sizes: "
        f"{minimum_combination_size} to "
        f"{maximum_combination_size}"
    )
    print(
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate single, pair and three-condition "
            "QRF train/test splits and overwrite "
