...
# Generate/refresh the augmented source automatically before the QRF sweep.
...
OUTPUT_RELATIVE_PATH = Path(
    "ml_result/experiments_per_group/qrf_experiments_per_group.parquet"
)

# Existing QRF runner/configuration file.
QRF_DRIVER_FILENAME = "3_1_run_qrf_interval_predictor.py"
...
# ============================================================
# Existing QRF driver loader
# ============================================================

def load_qrf_driver(project_root: Path):
    """
    ...
    """
    driver_path = project_root / QRF_DRIVER_FILENAME

    if not driver_path.exists():
        raise FileNotFoundError(
            f"QRF driver was not found: {driver_path}"
        )
    ...
    if spec is None or spec.loader is None:
        raise ImportError(
            f"Could not load QRF driver from {driver_path}"
        )
...
def extract_requested_metrics(
    ...
    """
    Extract the requested QRF metrics.
    ...
    if missing:
        raise KeyError(
            "QRF result is missing required aggregate metrics: "
    ...
    if not isinstance(predictions_df, pd.DataFrame):
        raise TypeError(
            "QRF result['predictions_df'] is required to compute "
            "r2, bias, and std, but it is missing or is not a DataFrame."
        )

    if predictions_df.empty:
        raise ValueError(
            "QRF result['predictions_df'] is empty."
        )
...
def build_curated_split_by_axis(
    ...
    for axis in ("main", "secondary"):
        ...
        split_config["split_selection_mode"] = (
            "best_rank_experiments_per_group"
        )
...
def generate_augmented_source(
    ...
    """
    Generate/refresh the augmented geometry before the QRF sweep.
    ...
    logger.info(
        "Generating augmented data before QRF sweep | "
...
def main() -> None:
    ...
    bending_setups_path = (
        project_root
        / qrf_driver.BENDING_SETUPS_PATH
    ).resolve()
    ...
    # --------------------------------------------------------
    top_splits_by_axis = (
        qrf_driver.load_top_qrf_splits_by_axis(
            project_root=project_root,
        )
    )
    ...
    tuned_params_by_axis = (
        qrf_driver.load_tuned_qrf_params_by_axis(
            project_root=project_root,
    ...
    if tuned_params_by_axis is None:
        # Match the behavior of the existing QRF driver:
        ...
        # selected augmented source.
        if AUGMENTED_SOURCE_NAME not in qrf_driver.BEST_QRF_PARAMS_BY_DATASET:
            raise KeyError(
                "No hard-coded best QRF configuration exists for "
                f"AUGMENTED_SOURCE_NAME={AUGMENTED_SOURCE_NAME!r}. "
                "Available sources: "
                f"{sorted(qrf_driver.BEST_QRF_PARAMS_BY_DATASET)}"
            )

        best_dataset_params = dict(
            qrf_driver.BEST_QRF_PARAMS_BY_DATASET[
    ...
    logger.info(
        "Best QRF params | main=%s",
        tuned_params_by_axis["main"],
    )
    logger.info(
        "Best QRF params | secondary=%s",
    ...
    excluded_experiments = {
        int(exp_id)
        for exp_id
        in qrf_driver.QRF_EXCLUDED_EXPERIMENTS
    }
    ...
    for experiments_per_group in range(
        ...
        logger.info(
            "Running QRF experiment | experiments_per_group=%s",
            experiments_per_group,
        )
        ...
        with tempfile.TemporaryDirectory(
            ...
            model_params_by_source = (
                qrf_driver.build_model_params_by_source(
                    geometry_sources=(
                        geometry_sources
                    ),
                    tuned_params_by_axis=(
            ...
            results = qrf_driver.run(
                project_root=project_root,
                geometry_sources=geometry_sources,
                splits_by_source=splits_by_source,
                feature_columns=list(
                    qrf_driver.QRF_FEATURE_COLUMNS
                ),
                excluded_experiments=list(
                    qrf_driver.QRF_EXCLUDED_EXPERIMENTS
                ),
                model_params_by_source=(
                    model_params_by_source
                ),
