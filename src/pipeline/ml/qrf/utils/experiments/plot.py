...
def plot_main_secondary_convergence_with_ranking(
    ...
    """
    Plot the effect of one varying QRF parameter for the top and worst
    ranked splits of the main and secondary axes.

    Parameters
    ----------
    metrics_df:
        Experiment metrics. It must contain one row for every split,
        axis and tested parameter value.

    split_metadata:
        DataFrame or path to various_splits.parquet containing the
        axis-specific QRF rankings.

    metric:
        Metric displayed on the y-axis.

    x_parameter:
        Hyperparameter displayed on the x-axis. Examples:
            n_estimators
            max_depth
            min_samples_leaf
            min_samples_split
    ...
    -------
    pd.DataFrame
        Ranking explanation for the selected top and worst splits.
    """
    ...
    plot_metrics_df[metric] = pd.to_numeric(
        plot_metrics_df[metric],
        errors="coerce",
    )

    plot_metrics_df = plot_metrics_df.dropna(
        subset=[
            "split_index",
            "axis",
            x_parameter,
    ...
    def select_extremes(
        ...
        top_df = axis_ranking_df.head(
            top_n
        ).copy()
        top_df["rank_group"] = "Top"

        worst_df = axis_ranking_df.tail(
            worst_n
        ).copy()
        worst_df["rank_group"] = "Worst"
    ...
    is_categorical = (
        x_parameter in categorical_parameters
        or not pd.api.types.is_numeric_dtype(
            plot_metrics_df[x_parameter]
        )
    )

    global_category_values: list[object] = []
    ...
    if is_categorical:
        ...
        category_to_position = {
            value: position
            for position, value
            in enumerate(global_category_values)
        }
    else:
        category_to_position = {}
    ...
    def draw_axis(
        ...
        plot_df = plot_metrics_df[
            plot_metrics_df["axis"].eq(axis)
            & plot_metrics_df["split_index"].isin(
                selected_indices
            )
        ].copy()
        ...
        for (
            ...
            if is_categorical:
                split_df["_x_position"] = (
                    split_df[x_parameter]
                    .map(category_to_position)
                )
                ...
                split_df[x_parameter] = pd.to_numeric(
                    split_df[x_parameter],
                    errors="coerce",
                )
            ...
            short_split_name = _shorten_text(
                split_name,
                max_length=max_label_length,
            )
    ...
    combined_metric_values = pd.concat(
        [
            ...
            plot_metrics_df.loc[
                plot_metrics_df["axis"].eq("secondary")
                & plot_metrics_df["split_index"].isin(
    ...
    table_ax.set_title(
        (
            "Axis-specific QRF split ranking. "
    ...
    figure.suptitle(
        (
            f"QRF {x_label} trend and "
            "axis-specific split ranking"
        ),
        fontsize=15,
    )
